document.addEventListener("DOMContentLoaded", () => {
    ...
    function trier(tri){
        switch(tri){
            ...
            case "1"://chronologique
                ...
                break;
            case "2"://alphabétique
                demandes.sort((a, b) =>a.forum.nom_forum.localeCompare(b.forum.nom_forum));
                break;
            default:
                break;
        }
    ...
    function populateListeDemandes() {
        ...
        if (!Array.isArray(demandes) || demandes.length === 0){
            const empty = creerElement('p', {classes:['border', 'p-3', 'rounded', 'text-gray-300'],
                text: `Il n'y a aucune demande d'adhésion en attente.`
            })
        ...
        demandes.forEach(d => {
            ...
            const div = creerElement('div', {
                classes: ['border', 'rounded', 'text-white', 'p-5', 'flex', 'flex-col', 'md:flex-row', 'items-center', 'justify-between'],
                dataset: { demandeId: d.id }
            });

            // forum name
            const p = creerElement('p', { text: d.forum.nom_forum });
            div.appendChild(p);
            ...
            const raisonInfo = creerElement('p', {
                classes: ['text-sm', 'text-gray-400', 'italic'],
                text: `Raison : ${d.raison ?? 'Aucune raison fournie.'}`
            });

            div.appendChild(userInfo);
            div.appendChild(raisonInfo);
            if (canManageDemandes){
                div.addEventListener("click", addToForum);
            }
        }
    ...
    function addToForum(evt){
        const id = evt.target.dataset.demandeId;
        d = demandes.find(d=>d.id == id);
        Swal.fire({
                title: "Confirmation",
                text: `Voulez-vous ajouter ${d.user.prenom} ${d.user.nom} au forum ${d.forum.nom_forum}?`,
                icon: "warning",
                ...
                if (result.isConfirmed) {
                    await addUser(d.user.id, d.forum.id);
                } else {
                    return;
                }
            });
    ...
    async function addUser(userId, forumId) {
        try {
                const response = await fetch(`/forums/addUser/${encodeURIComponent(userId)}/${encodeURIComponent(forumId)}`, {
                    method:"PUT",
                    headers: {
    ...
    async function supprimer(id){
         const route = canManageDemandes
                ? '/forums/demandes/suppression'
                : '/forums/demandes/annulation';
        Swal.fire({
                title: "Confirmation",
